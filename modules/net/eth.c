...
/*---------------------------------------------------------------------------*/
/*                                                                           */
/* Function Name   : eth_crc_calc                                            */
/* Description     : Calculate the Ethernet FSC                              */
/* Notes           :                                                         */
/*                                                                           */
/*---------------------------------------------------------------------------*/
u32      eth_crc_calc (void *pkt, usys  size)
{
    u8 *buf = (u8 *) pkt ;
    u32 crc = ~0, i ;
    ...
    return ~crc;
} /* End of function eth_crc_calc() */



/*---------------------------------------------------------------------------*/
/*                                                                           */
/* Function Name   : eth_hdr_init                                            */
/* Description     : Initialize the Ethernet Frame Header                    */
/* Notes           :                                                         */
/*                                                                           */
/*---------------------------------------------------------------------------*/
usys     eth_hdr_init (net_inst_t *net_inst, pkt_t *pkt, u8 *dst_mac,
                       u16 eth_type)
{
    eth_hdr_t *eth_hdr ;

    eth_hdr = (eth_hdr_t *) pkt ;

    /* Set the Destination */
    memcpy (eth_hdr->dst_mac, dst_mac, 6) ;    

    /* Set the Source MAC */
    memcpy (eth_hdr->src_mac, net_inst->mac, 6) ;

    /* Set the Type */
    eth_hdr->eth_type = htons(eth_type) ;

    return GOOD ;
} /* End of function ether_hdr_init() */
